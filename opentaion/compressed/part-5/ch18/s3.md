...
Quinn 可以访问 TEA 模块中的 34 种测试模式——这是针对常见测试挑战经过验证的方法库。对于 OpenTalon，有四种模式最为直接适用。本节具体逐一讲解。
### 模式：认证流程测试

认证流程有一个独特挑战：快乐路径很容易测试，但错误路径才是 bug 藏身之处。认证流程测试（Auth Flow Testing）模式涵盖了任何需认证的端点的完整状态集：
- 有效、未过期的会话 → 正确响应
- 无会话 Token → 401
...
代理端点必须符合 OpenAI 聊天补全规范，因为 CLI 用的是 OpenAI 客户端库。响应格式的偏差会导致客户端解析错误，而这种错误很难诊断。
...
用量计量有两种失败模式：Token 未被记录（静默数据丢失），以及 Token 被错误记录（错误的模型，错误的计数）。两者都需特定的测试。
```python
...
认证流程测试模式对基于时间的流程有一个特例：测试魔法链接是否正确过期。魔法链接是一个 Token，应该只能用一次，并在设定时间后过期。测试这两个属性需时间操纵：
```python
...
在 OpenTalon 中，Quinn 在测试套件中用了全部四种模式。Auth Flow 模式覆盖会话验证逻辑。API Contract 模式覆盖代理端点。Usage Metering 模式覆盖 Token 记录。Magic Link 模式覆盖认证回调。它们共同产生了一个验证组件间集成的测试套件——不仅验证每个端点返回 200，而且验证正确的数据从请求到数据库再到仪表盘正确地流经系统。