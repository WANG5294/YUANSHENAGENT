...
OpenTalon 中，上下文管理器不是一个外围工具——它是使代理循环对于长会话可持续的组件。这里的测试套件是未来每个对`context.py`的修改都必须满足的规范。当第 12 章添加上下文压缩功能（历史记录过大时进行汇总）时，现有测试将确认压缩不会破坏截断或系统提示保留。测试持久存在；实现不断演变。

### 刚才发生了什么

上下文管理器已实现、测试并提交。测试套件指定了四个行为属性，及一个使用 Hypothesis 的基于属性的测试，该测试生成数百个随机消息序列并验证不变量对所有序列都成立。
...
第 11 章从这些测试已就位开始，并每个新组件添加时扩展套件。
