...
TDD 循环有三个阶段：RED（一个指定所需行为的失败测试）、GREEN（使测试通过的最小实现）和 REFACTOR（保持测试通过的改进代码）。对人工开发者来说，这些阶段在单个上下文中发生，具有连续的记忆。当 Claude Code 作为实现者时，每个阶段都需要明确的指令来过渡。

本节追踪 OpenTalon 一个组件——OpenRouter 客户端中的重试逻辑——的完整循环，从失败测试到干净的实现。

### RED：失败测试

在编写任何实现代码之前，先编写测试。该测试指定了确切所需的行为。
...
这是 RED 状态。测试失败是因为 `opentaion.llm` 不存在。失败原因确认了测试本身是正确的——它不是因为错误的断言而失败，而是因为实现缺失。
...
GREEN 实现通过了测试，但存在问题：`CompletionResponse` 类很单薄，可以是一个数据类；重试休眠依赖于 `Retry-After` 头但没有合理的默认值；httpx 客户端在每次调用时重新创建。
...
该循环产生两次提交：一次在 GREEN（工作实现，可能粗糙），一次在 REFACTOR（干净实现）。两者都是检查点。如果重构破坏了什么，`git reset` 回到 GREEN 提交。测试套件是仲裁者——如果测试通过，状态就是安全的；如果失败，说明出了问题。

### 与 OpenTalon 的关联

在 OpenTalon 中，这个确切的循环——编写测试，确认 RED，实现到 GREEN，重构，在每个阶段提交——为 CLI 中的每个组件运行：上下文管理器、OpenRouter 客户端、工具集和代理循环本身。测试在组件存在之前就存在，因此从第一次迭代开始，每个实现都是可验证的。到第 10.5 节时，测试套件已经足够全面，可以作为第三部分其余部分的回归基线。
