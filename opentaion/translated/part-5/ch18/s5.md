...
第 12 章引入了黄金集（Golden Set）作为开发过程的回归测试套件——用于验证智能体开发系统是否仍然正常工作的任务。那个黄金集针对的是 CLI，而 CLI 是当时唯一存在的组件。

现在系统已经完整：CLI、API 和 Web 平台在生产环境中协同工作。黄金集也随之扩展。
...
该脚本在两分钟内运行完毕。所有四项任务都应通过。如果有任何任务失败，在继续任何部署或功能开发之前进行调查。
